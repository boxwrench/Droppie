...
export async function startGame(stage:(s:string)=>void,fail:(e:unknown)=>void) {
  ...
  const reset=()=>{sound.stopFacilities();input.clear();rig.reset();body.reset();input.recenter();baby.resetFace();physicsClock.reset();};
  ...
  const frame=(time:number)=>{
    ...
    try {
      ...
      if(!body.sleeping||renderedSurface!==body.surfaceRevision||renderedFace!==faceVersion||
  ...
  const dispose=()=>{
    ...
    facilityShadows.dispose();composite.dispose();baby.dispose();table.dispose();environment.dispose();optics.dispose();renderer.dispose();
  };
